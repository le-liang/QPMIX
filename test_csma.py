...
def run_MultiAgent(args, numOCA=1, numCSMA=1, w=1, traffic_type = [], packet_length=[], reward_Option = 'onePlus'):
    currentPath = args.dir + '/numOCA={}'.format(numCSMA)
    ...
    # plot.plotReward()
    # plot.plotThoughput()
    meanDelay, delayJitter, mean_delays_per_STA = plot.CSMA_delay(CSMA_CA_path, STA)
    ...
    # plot.OCA_delay_cdf(currentPath, STA)
    # plot.CSMA_delay_cdf(CSMA_CA_path, STA)
    plot.plot_CDF(currentPath, CSMA_CA_path, STA)
    # plot.plot_unsaturated_traffic(currentPath, CSMA_CA_path, STA)
